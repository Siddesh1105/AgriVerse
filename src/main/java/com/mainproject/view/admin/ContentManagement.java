 ...
 *
 * Content Management screen: sidebar, header, six summary cards, tab bar,
 * filter bar, content table, right hand info panel and a recent activity
 * strip.
 *
 * This follows the same pattern as the project's other admin screens
 * (UserManagement, ProductManagement, etc.): it is handed the shared
 * Stage plus a reference back to AdminDashboard, and show() swaps the
 * Scene's root to this screen. Calling adminDashboard.showDashboard()
 * swaps back.
 */
public class ContentManagement {
    ...
    // same palette as AdminDashboard so the screens look consistent
    ...
    public ContentManagement(Stage stage, AdminDashboard adminDashboard) {
        this.stage = stage;
        this.adminDashboard = adminDashboard;
        loadSampleData();
    }

    /** Builds the screen and puts it on the shared stage, same as the other admin screens. */
    public void show() {
    ...
    private VBox buildSidebar() {
        ...
        logoBox.setOnMouseClicked(e -> adminDashboard.showDashboard());
        ...
        subNav.setPadding(new Insets(6, 8, 10, 30));
        ...
        sidebar.getChildren().addAll(logoBox, navBox, subNav);
        return sidebar;
    }
    ...
    private Button buildNavButton(String label) {
        ...
        boolean active = label.equals("Content Management");
        if (active) {
        ...
        button.setOnAction(e -> onSidebarItemClicked(label));
        return button;
    }
    ...
    private HBox buildSubNavItem(String text, boolean active) {
        ...
        label.setTextFill(active ? Color.WHITE : Color.web("#c8e6c9"));
        if (active) label.setStyle("-fx-font-weight: bold;");
        box.getChildren().add(label);
        box.setOnMouseClicked(e -> selectTab(text));
        return box;
    }

    // sidebar handlers - this is the important bit: "Dashboard" (and the logo) hand
    // control back to AdminDashboard instead of showing a placeholder.

    private void onSidebarItemClicked(String item) {
        if (item.equals("Content Management")) {
            return; // already here, do nothing
        }
        // try a dedicated screen first (Product Management, User Management, ...);
        // if there isn't one yet (Dashboard, Analytics & Reports, ...) just go back
        // to the main dashboard
        boolean handled = adminDashboard.navigateTo(item);
        if (!handled) {
    ...
    private HBox buildHeader() {
        ...
        header.setStyle("-fx-background-color: white; -fx-border-color: #eaeaea; -fx-border-width: 0 0 1 0;");
        ...
        profileItem.setOnAction(e -> infoAlert("Profile", "Opening admin profile page."));
        settingsItem.setOnAction(e -> infoAlert("Settings", "Opening settings page."));
        logoutItem.setOnAction(e -> confirmLogout());
        profileMenu.getItems().addAll(profileItem, settingsItem, logoutItem);

        header.getChildren().addAll(menuButton, globalSearch, spacer, bell, mail, profileMenu);
        return header;
    }
    ...
    private HBox buildPageTitleBar() {
        ...
        Label subtitle = new Label("Manage all content, pages, banners and announcements");
    ...
    private VBox statCard(String icon, String label, Label valueLabel, String delta, String iconBg) {
        ...
        deltaLabel.setTextFill(Color.web("#2e7d32"));
    ...
    private HBox buildMainBody() {
        ...
        left.setStyle("-fx-background-color: white; -fx-background-radius: 12;"
                + "-fx-effect: dropshadow(gaussian, rgba(0,0,0,0.08), 8, 0, 0, 2);");
    ...
    private void selectTab(String name) {
        if (tabsBarRef == null) return; // called from the sub nav before the main body exists yet
        selectedTab = name;
    ...
    private VBox buildStatusCard() {
        ...
        for (String[] row : rows) {
            ...
            count.setFont(Font.font("Segoe UI", FontWeight.BOLD, 13));

            line.getChildren().addAll(dot, name, spacer, count);
            line.setOnMouseClicked(e -> {
                statusFilter.setValue(row[0]);
                applyFilters();
            });
