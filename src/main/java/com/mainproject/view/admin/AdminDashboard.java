...
public class AdminDashboard extends Application {
        ...
        // rebuilds the whole dashboard screen and puts it back on the shared stage -
        // other screens (like UserManagement, ContentManagement) call this to come
        // back here
        public void showDashboard() {
                rootLayout = new BorderPane();
                rootLayout.setStyle("-fx-background-color: " + BG + ";");
        ...
        // Called by nav buttons inside AdminDashboard itself. It keeps the sidebar
        // highlight in sync and then hands off to navigateTo() for the actual
        // routing, so the routing table only exists in one place.
        private void switchPage(String pageName, Button clickedButton) {
                if (navigateTo(pageName)) {
                        // navigateTo() already swapped to a different screen/class,
        ...
        // Central place that knows how to reach every admin screen. Public so any
        // other screen (ContentManagement, UserManagement, ...) can call
        // adminDashboard.navigateTo("Product Management") directly instead of only
        // being able to get back to the plain Dashboard.
        // Returns true if it navigated to a dedicated screen, false if the caller
        // should fall back to Dashboard/placeholder handling.
        public boolean navigateTo(String pageName) {
                if (pageName.equals("User Management")) {
                        new UserManagement(primaryStage, this).show();
                        return true;
                }
                ...
                // "Dashboard", "Analytics & Reports" and anything not built yet fall
                // through so the caller can show the Dashboard/placeholder instead
                return false;
        }
